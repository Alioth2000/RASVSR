...
class VideoRecurrentModel(VideoBaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            self.fix_fusion = opt['train'].get('fix_fusion')
        ...
        if self.fix_flow_iter:
            if current_iter == 1:
                ...
                for name, param in self.net_g.named_parameters():
                    ...
                    if self.fix_fusion:
                        if 'fusion' in name or 'deform_align' in name:
                            param.requires_grad_(False)
            elif current_iter == self.fix_flow_iter:
                # logger.warning('Train all the parameters.')
                logger.warning('Train flow network and edvr parameters.')
                for name, param in self.net_g.named_parameters():
                    if 'spynet' in name or 'edvr' in name:
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        if with_metrics:
            if not hasattr(self, 'metric_results'):  # only execute in the first run
                self.metric_results = {}
                if type(dataset).__name__ == 'VideoRecurrentTestSVSRPreDataset':
                    num_frame = 1
            ...
            if i < num_folders:
                for idx in range(visuals['result'].size(1)):
                    ...
                    if 'gt' in visuals:
                        ...
                        h_sub = gt_img.shape[0] % 16
                        w_sub = gt_img.shape[1] % 16
                        gt_img = gt_img[:gt_img.shape[0] - h_sub, :gt_img.shape[1] - w_sub, :]
                        # uint8, bgr
